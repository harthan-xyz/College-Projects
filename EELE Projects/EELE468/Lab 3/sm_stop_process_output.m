...
d = squeeze(mp.Avalon_Sink_Data.Data);    % data
c = squeeze(mp.Avalon_Sink_Channel.Data); % channel
v = squeeze(mp.Avalon_Sink_Valid.Data);   % valid
left_index = 1;
right_index = 1;
for i=1:length(v)
    if v(1) == 1 % check if valid, valid is asserted when there is data
        if c(1) == 0 % if the channel number is zero, it is left channel data
            mp.left_data_out(left_index) = double(d(i));
            mp.left_time_out(left_index) = t(i);
            left_index                   = left_index + 1;
        end
        if c(1) == 1 % if the channel number is one, it is right channel data
