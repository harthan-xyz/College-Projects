/*  @file
    This kernel driver controls a Dual Port RAM block on a DE10-Nano device
...
static ssize_t DPRAM_read(struct file *file, char *buffer, size_t len, loff_t *offset);
static ssize_t DPRAM_write(struct file *file, const char *buffer, size_t len, loff_t *offset);
static int DPRAM_open(struct inode *inode, struct file *file);
static int DPRAM_release(struct inode *inode, struct file *file);
static long DPRAM_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
...
static ssize_t register_wr_dout_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
...
DEVICE_ATTR (control_wr,    	0644,   control_wr_read,    	control_wr_write);
DEVICE_ATTR (control_data,      0664,   control_data_read,      control_data_write);
DEVICE_ATTR (register_wr_dout,  0664,   register_wr_dout_read,  register_wr_dout_write);
...
static int DPRAM_init(void) {
  ...
  // Test call to see if device is recognized by the system
  //ret = platform_driver_probe(&DPRAM_platform, DPRAM_probe);
...
// Probe function of platform device driver
static int DPRAM_probe(struct platform_device *pdev) {
  ...
  status = device_create_file(device_obj, &dev_attr_control_wr);
  if (status)
...
// Error messages for probe function  
...
bad_device_create_file_2:
  device_remove_file(device_obj, &dev_attr_control_wr);
...
static ssize_t control_wr_read(struct device *dev, struct device_attribute *attr, char *buf) {
  fe_DPRAM_dev_t * devp = (fe_DPRAM_dev_t *)dev_get_drvdata(dev);
  fp_to_string(buf, devp->control_wr, 0, true, 9);
  strcat2(buf,"\n");
  return strlen(buf);
}
...
static ssize_t control_wr_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
  ...
  substring[substring_count] = 0;
  tempValue = set_fixed_num(substring, 0, false);
  devp->control_wr = tempValue;
  iowrite32(devp->control_wr, (u32 *)devp->regs + 1);
  return count;
}
...
static ssize_t register_wr_dout_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
  ...
  devp->register_wr_dout = tempValue;
  iowrite32(devp->register_wr_dout, (u32 *)devp->regs + 3);
  return count;
}
...
static int DPRAM_open(struct inode *inode, struct file *file) {
  	printk("DPRAM : Device OPEN\n");
	return 0;
}

static int DPRAM_release(struct inode *inode, struct file *file) {
  	printk("DPRAM : Device RELEASED\n"); // Test
	return 0;
}

static long DPRAM_ioctl(struct file *file, unsigned int cmd, unsigned long arg){
	printk("DPRAM : Device IOCTL\n"); // Test
	return 0;
}

static ssize_t DPRAM_read(struct file *file, char *buffer, size_t len, loff_t *offset) {
    
	uint8_t *data = "The driver has entered the read function.\n";
